...
void render_2(t_particula *particulas, long int N_PARTICULAS, int frame) {
...
void atualiza_particulas_2(double dt, int n_dim, double* velsx, double* velsy, t_particula* particulas, long int N_PARTICULAS, int frame) {
  int tam_p = 0;
  for(long int i = 0; i < N_PARTICULAS; i++) {
    ...
    double pvelx = velsx[((int)mirror_value(p.y, MIRROR_WIDTH)+OFFSET_Y)*(imax+2) + (int)p.x+OFFSET_X];
    double pvely = velsy[(int)mirror_value(p.y, MIRROR_WIDTH)*(imax+2) + (int)p.x];

    particulas[i].vx = pvelx;
    particulas[i].vy = pvely;

    particulas[i].x += pvelx * dt;

    if(particulas[i].x < 1) particulas[i].x = 1;
    if(particulas[i].x > n_dim-1) particulas[i].x = n_dim-1;

    particulas[i].y += pvely * dt;
  ...
  render_2(particulas, N_PARTICULAS, frame);
}
...
int main(int argc, char ** argv){
	int n_dim = WIDTH, nt = 20000, stable_max_time = 30;
  ...
  // gettimeofday(&start, NULL);
    ...
    atualiza_particulas_2(dt, n_dim, vx, vy, particulas, N_PARTICULAS, frames);
		// Desenhando
    ...
    glfwPollEvents();
		...
		ftime(&start);
		milliseconds = (int) (1000.0 * (end.time - start.time) + (end.millitm - start.millitm));
		if(milliseconds/1000 > stable_max_time) break;
	}
