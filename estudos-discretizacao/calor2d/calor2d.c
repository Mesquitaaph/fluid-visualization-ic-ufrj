...
int main() {
  ...
  //   for(int i = 0; i < n; i++) {
  //     estados[t][i] = (double*) malloc(n*sizeof(double));
  //   }
  // }
  ...
  // print_matrix(estados[2], n, n);
  // printf("\n");
  // print_matrix(estados[8], n, n);
  // for(int t = 0; t < nt; t++) {
  //   printf("Vector t%d = \n", t);
  ...
  // DEIXANDO AQUI EMBAIXO COMENTADO PRA VER DEPOIS
  // char initSim;
  // scanf("%c", &initSim);
  int frame = 1;
  while (!glfwWindowShouldClose(window)){
    ...
    for(int i = 1; i < n-1; i++) {
      for(int j = 1; j < n-1; j++) {
        // nova_temp(n, estados[frame-1], estados[frame], i, j, r);
