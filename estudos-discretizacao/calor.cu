...
// Função que recebe a matriz A quadrada nxn e o vetor b nx1 do sistema AX = b
// e obtém a solução X utilizando a decomposição LU, retornando-a.
void SOLVE_LU_CONC(int n, double** L, double** U, double* b, double* X) {
  double *c = (double*) malloc(n*sizeof(double));
...
void solveMatrix(int dim_mat, double** m, double* b, double* X) {
  ...
  L = (double**) malloc(dim_mat * sizeof(double*));
  U = (double**) malloc(dim_mat * sizeof(double*));
  if(L == NULL || U == NULL) {
  ...
  for(int i = 0; i < dim_mat; i++) {
    U[i] = (double*) malloc(dim_mat * sizeof(double));
    for(int j = 0; j < dim_mat; j++) {
      if(j >= i) {
        U[i][j] = src_m[i*dim_mat + j];
      } else {
        U[i][j] = 0.0;
      }
    }
  ...
  for(int i = 0; i < dim_mat; i++) {
    L[i] = (double*) malloc(dim_mat * sizeof(double));
    for(int j = 0; j < dim_mat; j++) {
      if(j < i) {
        L[i][j] = src_m[i*dim_mat + j];
      } else if(j == i) {
        L[i][j] = 1.0;
      } else {
        L[i][j] = 0.0;
