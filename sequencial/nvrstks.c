...
int final_iter = 0;
int execute(int argc, char ** argv, FILE *fp) {
	int set_time=0;
	...
	while(!state){
		...
		ftime(&end);
		int milliseconds = (int) (1000.0 * (end.time - start.time) + (end.millitm - start.millitm));
		// printf("Tempo decorrido %lf segundos (%lf minutos) - Iteracao %d\r", milliseconds/1000.0, milliseconds/1000.0/60.0, iter);
		printf("Iteracao %d/%d\r", iter, final_iter);
	}
	printf("\n");
	set_bondCond();
	set_lidDrivenCavityProblem(1.0);
	...
	ftime(&end);
	int milliseconds = (int) (1000.0 * (end.time - start.time) + (end.millitm - start.millitm));
	printf("Tempo decorrido %lf segundos (%lf minutos) - Iteracao %d\n\n", milliseconds/1000.0, milliseconds/1000.0/60.0, iter);
	fprintf(fp, "Tempo decorrido %lf segundos (%lf minutos) - Iteracao %d\n\n", milliseconds/1000.0, milliseconds/1000.0/60.0, iter);
	// write_file(argv[2]);
	free_matrix();
	final_iter = iter;
	return milliseconds;
...
int main(int argc, char ** argv){
	...
	int t1 = execute(argc, argv, fp);
	int t2 = execute(argc, argv, fp);
	int t3 = execute(argc, argv, fp);

	double media_seconds = (t1 + t2 + t3)/(3.0*1000.0);

	fprintf(fp, "\neps = %lf, tau = %lf | ", eps, tau);
	fprintf(fp, "Media %lf segundos (%lf minutos)\n\n", media_seconds, media_seconds/60.0);
